...
if($_SERVER['REQUEST_METHOD']=="POST")
{
	if(empty($_POST["name"]))
	{
		...
		if(!preg_match("/^[a-zA-Z ]*$/", $name)){
			$name_error = "Chỉ cho phép chữ cái và khoảng trắng";
		}
	}
	...
	//chech if url address syntax is valid (this regular expresstion also allows dashes in the url)
		if(!preg_match(" #\bhttps?://[^\s()<>]+(?:\([\w\d]+\)|([^[:punct:]\s]|/))#",$url)){
			$url_error = "Url không hợp lệ";
		}
	}
